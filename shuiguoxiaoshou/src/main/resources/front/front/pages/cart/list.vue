				...
				<view class="glance-shop-cart-scrollx-items" style="display: inline-block;width: 100%;">
					<view class="glance-shop-cart-scrollx-items-item">
						...
						<view style="width: 30%;height: 100%;text-align:center;">
							<image :src="item.shuiguoPhoto" mode="scaleToFill" style="height: 75px;width: 75px;line-height: 80px;padding-top: 5px;"></image>
						</view>
						<!-- 描述 -->
						<view class="glance-shop-cart-items-item-des">
							<!-- 名称 -->
							<view class="sigle-line-text" style="font-size: 16px;height: 33.33%;text-align: left;">{{ item.shuiguoName }}</view>
							<!-- 属性 -->
							<!-- 价格 & 数量-->
							<view class="glance-shop-cart-items-item-pq">
								<view class="sigle-line-text" style="font-size: 15px;text-align: left;width: 50%;">￥{{item.shuiguoNewMoney}}</view>
								<!-- 数量操作 -->
		...
		methods: {
			async init() {
				...
				for (let i = 0; i < this.cart.length; i++) {
					// 总金额 
					this.totalamount = parseFloat(this.totalamount) + parseFloat(this.cart[i].shuiguoNewMoney * this.cart[i].buyNumber)
					// 总数量
					...
					if (this.cart) {
						for (var i = 0; i < this.cart.length; i++) {
							...
							if (this.isselected(this.cart[i].id)) {
								...
								// 更新总金额
								this.totalamount = this.totalamount - this.cart[i].shuiguoNewMoney * this.cart[i].buyNumber
								this.totalamount = this.fmamount(this.totalamount)
							}
						}
					}
				...
				for (let i = 0; i < this.cart.length; i++) {
					if ((this.cart[i].id == itemid) && (this.cart[i].buyNumber > 0)) {
						...
						if (this.isselected(this.cart[i].id)) {
							...
							// 更新总金额
							this.updatetotalamt(-this.cart[i].shuiguoNewMoney)
							// 执行update接口
						...
						if (this.cart[i].buyNumber == 0) {
							...
							uni.removeStorageSync(`cart${this.cart[i].shuiguoId}`)
							this.init();
						}
						return
					}
