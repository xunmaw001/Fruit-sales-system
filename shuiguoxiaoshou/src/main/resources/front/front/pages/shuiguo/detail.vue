		...
		<view class="container">
			...
			<view class="detail-content">
				<view class="price detail-list-item priceFavor"
					...
					<view class="text" style="color: red;textAlign:left">
						<text style='font-size:20rpx'>¥</text><text style="font-weight:bold;font-size:50rpx">{{detail.shuiguoNewMoney}}</text><text style="font-size: 20rpx;color: #8799A3;margin-left: 10rpx;">
							<text v-if="detail.shuiguoOldMoney!=null">原价￥<text style="text-decoration:line-through">{{detail.shuiguoOldMoney}}</text></text>
                    	</text>
					</view>
				</view>
					...
					<view class="detail-list-item"
						...
						<view class="lable"
							...
							水果库存：</view>
						<view class="text" :style='{"color":"#47EDB5","textAlign":"center","fontSize":"24rpx"}'>
							{{detail.shuiguoKucunNumber}}</view>
					</view>
					<view class="detail-list-item"
						  :style='{"boxShadow":"0 0 4rpx 0 #006633 insta","margin":"0 60rpx 0 60rpx","borderColor":"rgba(218, 218, 218, 1)","backgroundColor":"#fff","borderRadius":"0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"68rpx"}'>
						<view class="lable"
			...
			<view class="time-content"
				...
				<view class="content" style="line-height: 50upx;letter-spacing: 5upx;"
					...
					<rich-text :nodes="detail.shuiguoContent"></rich-text>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('shuiguo', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.shuiguoPhoto ? this.detail.shuiguoPhoto.split(",") : [];
			},
			//添加购物车
            async onCartTap() {
            ...
            onSubmit(){
                uni.setStorageSync('orderGoods', [{
                        shuiguoId: this.detail.id,
                        shuiguoName: this.detail.shuiguoName,
                        shuiguoPhoto: this.swiperList[0],
                    buyNumber: 1,
                        shuiguoNewMoney: this.detail.shuiguoNewMoney,
                }])
                this.$utils.jump('../shuiguoOrder/confirm');
            },
			...
			async upCallback(mescroll) {
				let res = await this.$api.list('shuiguoCommentback', {
					page: mescroll.num,
					limit: mescroll.size,
					shuiguoId: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
                this.$utils.jump(`../shuiguoCommentback/add-or-update?shuiguoId=${this.id}`)
            },
		}
	}
</script>
