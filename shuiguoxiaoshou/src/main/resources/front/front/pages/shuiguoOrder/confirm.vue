    ...
    <view class="content">
        <form>
            ...
            <view v-for="(item,index) in orderGoods " v-bind:key="index" class="cu-form-group">
                <image class="avator" :src="item.shuiguoPhoto" mode=""></image>
                <view class="title" style="width: 75%;">
                    <view style="margin-top: -50rpx;">{{item.shuiguoName}}</view>
                    <view >
                        x{{item.buyNumber}}
                        <text style="margin-left: 30upx;color: red;">￥{{item.shuiguoNewMoney}}</text>
                    </view>
                </view>
            </view>
            <view @tap="onAddressTap" class="cu-form-group">
                ...
                <view>
                    <text v-if="shuiguoOrderPaymentTypes == 1">原价：￥{{(maxNewMouey).toFixed(2)}}</text>
                    <view v-if="shuiguoOrderPaymentTypes == 1"></view>
                    <text v-if="shuiguoOrderPaymentTypes == 1">折扣价：￥{{(maxNewMouey * zhekou).toFixed(2)}}</text>
                    <text v-if="shuiguoOrderPaymentTypes == 2">{{(maxNewMouey).toFixed(2)}}积分</text>
                </view>
            </view>
        </form>
    ...
    export default {
        data() {
            return {
                ...
                maxNewMouey: 0,//总价格
                addresszhi: {},//选中的地址
                shuiguoOrderPaymentTypes:1,//是什么支付类型
                zhi:[
                    {
                        id:1,
                        val:"余额"
        ...
        async onLoad(options) {
            ...
            if(this.orderGoods.length>0){
                for (let i = 0; i < this.orderGoods.length; i++) {
                    this.maxNewMouey = this.maxNewMouey + parseFloat(this.orderGoods[i].shuiguoNewMoney) * this.orderGoods[i].buyNumber
                }
            }
                        ...
                        if (res.confirm) {
                            if(_this.addresszhi == null){
							 ...
							 let data = {
								 ...
								 shuiguos: JSON.stringify(_this.orderGoods),
								 yonghuId: _this.user.id,
								 shuiguoOrderPaymentTypes:  _this.shuiguoOrderPaymentTypes,
							 }
							await _this.$api.requestConditionDataGet('shuiguoOrder','order',null,data);
                            _this.$utils.jump('/pages/shuiguoOrder/list');
                        }
                    }
                });
            },
