        ...
        <form class="app-update-pv">
			<view
                ...
                <textarea
                        ...
                        v-model='ruleForm.shuiguoCommentbackText' placeholder="留言内容"></textarea>
            </view>
    ...
    export default {
        ...
        async onLoad(options) {
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`shuiguoCommentback`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.shuiguoId){
                this.ruleForm.shuiguoId = options.shuiguoId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
			if ((!this.ruleForm.shuiguoCommentbackText)) {
                this.$utils.msg(`留言内容不能为空`);
                return
            }
                if (this.ruleForm.id) {
                    await this.$api.update(`shuiguoCommentback`, this.ruleForm);
                } else {
                    await this.$api.save(`shuiguoCommentback`, this.ruleForm);
                }
